...
#
# Run on VPS: bash VERIFY_DOMAINS_SSL.sh
#
################################################################################
...
echo "╔════════════════════════════════════════════════════════════╗"
echo "║         Domain and SSL Verification Script                 ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
if command -v pm2 &> /dev/null; then
    if pm2 list 2>/dev/null | grep -q "online"; then
        ...
        pm2 list | grep -E "online|stopped" || echo "No apps found"
    else
...
if systemctl is-enabled certbot.timer 2>/dev/null; then
    ...
    systemctl status certbot.timer --no-pager | grep "Active" || true
else
