...
export default function ComparePage() {
  ...
  const steps = [
    ...
    },
    {
      ...
      description: "Choose the benefit period that works for you",
    },
    {
      title: "Your Contact Info",
      description: "We'll send personalized disability insurance quotes",
    },
  ];
  ...
  const handleBenefitPeriodChange = (value: string) => {
    setFormData({ ...formData, benefitPeriod: value });
  };

  const handleWaitingPeriodChange = (value: string) => {
    setFormData({ ...formData, waitingPeriod: value });
  };
  ...
  const handleNextStep = () => {
    if (currentStep === 0) {
      ...
      if (!formData.benefitPeriod) {
        setError("Please select benefit period");
        return;
      }
  ...
  if (completed) {
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-blue-50 to-green-50 py-12 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="card text-center mb-8">
            ...
            <p className="text-gray-600 mb-8">
              We found {quotes.length} disability insurance options for your income level. Compare them below or
              ...
              <div key={quote.id} className="card">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">{quote.benefit}</p>
                  </div>
          ...
          <div className="mt-12">
            ...
            <p className="text-gray-600 mb-6">
              If you're interested in promoting disability insurance, join these affiliate programs and earn
              commissions:
            </p>
            <AffiliateComparisonTable
              programs={getAffiliatesByNiche("disability")}
              siteName="Disability Insurance"
        ...
        <div className="card">
          ...
          {currentStep === 0 && (
            <div className="space-y-6">
              ...
              <div className="form-group">
                <label className="form-label">Approximate Monthly Income</label>
                <input
                  type="number"
          ...
          {currentStep === 1 && (
            <div className="space-y-6">
              <div className="form-group">
                <label className="form-label">What benefit period interests you?</label>
                {[
                  { value: "short_term", label: "Short-Term Disability", desc: "2-5 years" },
                  { value: "long_term", label: "Long-Term Disability", desc: "Until retirement" },
                  { value: "both", label: "Both Short & Long-Term", desc: "Maximum protection" },
                ].map((option) => (
                  <button
                    key={option.value}
                    onClick={() => handleBenefitPeriodChange(option.value)}
                    className={`w-full p-4 rounded-lg border-2 text-left transition-all mb-3 ${
                      formData.benefitPeriod === option.value
              ...
              <div className="form-group">
                <label className="form-label">Preferred waiting period before benefits begin?</label>
                {[
                  ...
                  <button
                    key={option.value}
                    onClick={() => handleWaitingPeriodChange(option.value)}
                    className={`w-full p-4 rounded-lg border-2 text-left transition-all mb-2 ${
                      formData.waitingPeriod === option.value
                        ? "border-blue-500 bg-blue-50"
                        : "border-gray-300 bg-white hover:border-blue-300"
                    }`}
                  >
