...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !email || !password || password.length < 8) return NextResponse.json({ message: 'Invalid' }, { status: 400 });
    ...
    return NextResponse.json({ message: 'Created', userId: user.id }, { status: 201 });
  } catch (error) {
